...
import { DIRECTOR_ASSET_ENTRIES } from '../packages/experimental/qingmu-imago-method-adapter/src/director-assets/registry.ts'
...
export function render(): string {
  ...
  const directorAssets = DIRECTOR_ASSET_ENTRIES.map((entry) => {
    const base = `packages/experimental/qingmu-imago-method-adapter/assets/director/${entry.repoId}/${entry.commit}`
    return `| [${entry.repoId}](${entry.repoUrl}) | \`${entry.commit}\` | [${entry.license}](${base}/LICENSE) | [scope and modifications](${base}/ADMISSION.md) | [SHA-256 ledger](${base}/PROVENANCE.sha256) |`
  })
...
## Inactive experimental director assets

These selected sources are checked in as inert references, not runtime dependencies or installed skills. ArcReel admission covers only its MIT-licensed \`skills/\` subtree; its AGPL application is excluded. Each package preserves its license, file hashes, and admission limitations. The [machine-readable inventory](packages/experimental/qingmu-imago-method-adapter/DIRECTOR_ASSET_SBOM.json) records the fixed upstream commits and local file hashes. It does not grant execution, production, or approval authority.

| Repository | Commit | License | Admission | Integrity |
| --- | --- | --- | --- | --- |
${directorAssets.join('\n')}
