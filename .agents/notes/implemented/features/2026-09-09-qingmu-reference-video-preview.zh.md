...
已有外部视频保留原上传回执。大小受限的辅助来源记录将已保存的输入、结果、下载记录绑定到视频原字节与当前镜头版本。这些记录无法证明哪次请求生成了视频，也无法验证平台执行；两项核验标记均为 false，登记不改变采用或审批。来源记录损坏时来源读取失败，但不使原上传回执失效。结果不明时使用原幂等键与请求摘要读取恢复，镜头编辑后的历史回执仍可恢复。
