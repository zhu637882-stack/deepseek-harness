...
After ordinary adoption and one real batch submission, refresh replay exposed the panel collapsing while being read. Reloading source ports reapplied the default expansion. Initialization now runs once per project/episode, preserves user toggles during refresh and opens initially for active work. The reproduced refresh test failed before the fix; all nine batch component tests pass after it.
