...
This extends the [reference routing decision](2026-09-10-qingmu-reference-director-routing.md): saved and intentionally manual prompt parts remain editable, while every new automatic preparation requires authored execution. The former optional single-shot argument silently copied the full source; the shared assembler and native tool now reject that omission just as batch preparation does.
...
System regression covers silent action, offscreen speech, product inserts and repeated lines through native save and adapter previews, alongside the batch suite exercising the same assembly and submission route. Source-aligned drafts with the former automatic full-source marker return to preparation; manual and current execution drafts remain reusable. Scene authoring now shows timed causal action beats in its output example and preserves them on adoption. Asset authoring specifies neutral, isolated prop references separately from intentional in-scene interaction views. These changes apply to future projects without customer settings or per-film patches. Read-only inspection of 72 shots from two existing advertising projects found historical inputs, mostly stale sources and monolithic action beats; enabled audio and bound speaker voices did not explain the reported audible defects. No old media was replaced. Prompt and transport tests cannot certify generated motion, identity, sound or continuity.
