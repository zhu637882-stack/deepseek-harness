...
Asset quotations use the same restricted detail handling for HTTP 409 and 422. A live scene edit with unsupported reference boxes otherwise hid the actionable model error. The adapter preserves a redacted string of at most 2048 characters, never exposes structured validation payloads or server errors, and sends no retry. A Loader browser-RPC snapshot covers this rejection without a paid generation; focused cases cover size, redaction and unrelated statuses.
