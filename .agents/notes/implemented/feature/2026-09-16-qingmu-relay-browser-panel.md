...
For an existing batch the panel exposes read/refresh, pause (`advanceRelayBatch`, enabled only while `running` with zero admissions, matching the controller's pause rule), lease recovery (`recoverRelayBatch`), completion, and closure with a mandatory operator-typed reason. Admitting a director is absent because a legitimate admission needs the Host's real `contextSnapshotSha256`, which the browser cannot author.
