...
1. 保存剧本与素材设计后打开“导演工作区”，选择一场文本场景，可让原生导演设计并采用到可编辑卡片；也可点击“建立本场镜头”手动填写。手动初始卡只复制原文动作/对白。
2. 填写镜头名称、叙事目的、画面描述、动作和时长（0.5–30 秒），核对对白分配。单次最多 64 个镜头，仍受请求大小限制。选择“预览保存影响”，再选择“确认保存规划”。
3. 保存的场景、对白人物和镜头拥有真实易梦 ID 及原始来源行绑定。选择一个镜头，修改字段并显式保存新结构版本。刷新或重启读回同一对象。参考媒体和首个合法 PromptIR 仍是独立前提；结构 Ready 不是内容批准。

保存一场后可切换到其他场次继续建立，镜头追加到本集末尾；已保存场次可切回编辑。未保存修改或待确认的保存结果会保留在当前场，处理后再切换。刷新保留已选择场次，不重复建立其他场次。
