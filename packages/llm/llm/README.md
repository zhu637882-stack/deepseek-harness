...
The `director-proposal` purpose is the same kind of explicit auxiliary classification. It lets a private Host request provider-specific strict JSON behavior through a prepared call without turning the call into an agent loop or changing ordinary conversation semantics.
