...
附件授权扫描允许扩展事件携带空值或标量。此类事件不授予图像引用；后续持久化引用仍只授权与之匹配的附件 ID。
