...
扩展事件可以携带空值或标量载荷。它们不提供显式 Turn/Step 坐标，像空对象一样继承周边位置；实时追加与历史重建都保留其后的节点。空载荷不同于含有 `turn: null` 的对象，后者明确属于会话层。
