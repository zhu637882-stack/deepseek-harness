...
`qingmu_read_director_plan` and `qingmu_save_director_plan` read and patch the complete canonical shot design through the existing Writer scene-planning API. Creative keys are extensible; script identities and provenance remain protected. A logged read binds script/storyboard revisions, and an identical request recovers the same durable save receipt after uncertainty. A verified save can advance the current native turn's context; unrelated source or selection changes cannot. Fractional action timings remain unchanged. Recovery failures report the adapter diagnostic without dispatching a save. These tools generate no media. Reference drafts still need reconciliation with the newly saved design before generation.
