...
预设作用域中的 `./skill-resources` 插件按清单路径读取创作参考、子技能、引擎和模板，返回来源提交、上游与适配后哈希和明确分页。文件变更、未登记路径、目录外符号链接及超大页均拒绝返回，不截断内容。原生 `skill` 负责技能入口读取。资料作为工具结果进入会话，不写项目或调用生成模型；调用方沿 `nextLine` 读完所需资料。嵌套 Markdown 资源同时返回 `linkedResources`，给出相对技能根目录的准确路径；路径缺失时提示匹配的已打包路径，不代读其他资源。原文及哈希保持不变。
...
读取已保存素材设计时，也向支持图像的导演交付当前页中设计明确引用的图片像素，按ID与哈希去重并保留各实体当前用途。缺失、过时或不支持的输入明确标出，不换用其他候选，不额外调用模型。历史出图描述仍可显式查看。图片按当前用途提供依据，不授予媒体验收。
...
批量创作会话可依据当前项目与分集请求读取这些候选，无需选中单镜。读取前核对镜头属于本集，并沿用同一套观察、评论和回执合同。这仍是只读操作，不将写作会话变成直接保存或生成会话。
...
`qingmu_read_scene_design` 可同时读取指定镜头的当前视频来源与精确引用，保留完整生成依据和逐字对白后缀，不重复创作档案或旧执行稿。批量准备由此可从分集索引开始，按页取得完整来源。
