...
export function registerDirectorPlanTools(ctx: Context, ports: Ports): void {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      cut: { type: 'json', required: true, description: 'Object with clips (frameId, assetId, sha256, inSec, outSec, optional sourceGainDb and reframe), audioCues (assetId, sha256, kind music/ambience/effect/dialogue, startSec on assembled film, inSec/outSec on source, gainDb -60..6, fadeInSec/fadeOutSec, optional gainPoints) and soundPlan string. gainPoints: 2..64 points {timeSec, gainDb}; strictly increasing assembled-film seconds within this cue, relative gain -60..6 dB added to cue gain. Values interpolate in dB and hold outside first/last points. Use paired points around dialogue for gradual ducking/recovery; omission preserves constant cue gain. reframe is an optional static crop {zoom:1..4, x:0..1, y:0..1}; x/y travel left/top to right/bottom within the remaining source, .5 centers. Omit to keep the original frame. Crop is rounded to even pixels before scaling, preserves timing/audio and reduces retained resolution; it cannot repair hidden world geometry. Cues cannot exceed source/film duration. Lowering sourceGainDb also lowers its dialogue and ambience; never treat it as music separation.' },
