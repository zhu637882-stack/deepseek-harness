...
export function registerDirectorPlanTools(ctx: Context, ports: Ports): void {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: { sourceAssetId: { type: 'string', description: 'Optional exact assetId from a previous cut read. Returns that current source with its listening URL; no save receipt or mutation.' },
      revisionId: { type: 'string', description: 'Optional exact revisionId from cutVersions to inspect a retained edit. Large reads default to the newest edit and list every retained version.' } }, output,
    ...
    async execute(args, exec) {
      ...
      if (args.revisionId === undefined && Buffer.byteLength(JSON.stringify(input), 'utf8') <= 47000) return input
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Save the assembled-film clips, independent sound cues and director sound plan to a retained editable revision. Optionally render a local MP4 when authorized. Music/ambience/effect/dialogue cues can span shots. soundPlan is explanatory text, not an instruction executed by the renderer: it cannot remove subtitles, change frameTextOverlays or repair source video pixels. Frame overlays belong to the bound shot director plan; use qingmu_read_director_plan and qingmu_save_director_plan within the correct shot scope, or report the missing scope. Verify the actual executable field after saving before claiming an edit happened. No paid model, source replacement or human signoff. Retry identical arguments after an uncertain result.',
    parameters: {
