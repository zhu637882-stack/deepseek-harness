...
it('carries resolved creation settings into the native design request before generating images', async () => {
  ...
  expect(storyPort.send.mock.calls[0]?.slice(0, 2)).toEqual([expect.any(String), expect.stringContaining('visualIdentity 保存稳定外观')])
  ...
  expect(storyPort.send.mock.calls[0]?.slice(0, 2)).toEqual([expect.any(String), expect.stringContaining('真实引用的 assetId/assetSha256')])
  expect(port.generateAssetImage).not.toHaveBeenCalled()
})
