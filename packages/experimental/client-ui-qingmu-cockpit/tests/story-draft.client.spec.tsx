...
import { readStoryDraft } from '../src/client/story-draft.ts'
...
it('does not send twice after an uncertain response or expose another project candidate', async () => {
  ...
  expect(screen.getByRole<HTMLButtonElement>('button', { name: '编剧处理中…' }).disabled).toBe(true)
  fireEvent.click(screen.getByRole('button', { name: '读取原编剧结果' }))
