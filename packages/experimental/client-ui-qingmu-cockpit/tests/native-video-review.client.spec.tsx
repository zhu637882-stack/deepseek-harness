...
it('shows heard evidence and the uncertain status without claiming approval', async () => {
  ...
  expect(view.container.textContent).toMatchInlineSnapshot('"音画检查对照导演设计检查当前视频，结果用于审看与返修。本版已检查刷新记录对白内容尚未确认本项没有可用的听觉证据。语气与表演无法确认音量不足，无法确认语气。环境底声需调整对白开始时雨声消失。2–4 秒空间声学尚未确认本项没有可用的听觉证据。动作拟音尚未确认本项没有可用的听觉证据。配乐衔接尚未确认本项没有可用的听觉证据。AI 检查供参考，最终是否采用由你决定。"')
})
