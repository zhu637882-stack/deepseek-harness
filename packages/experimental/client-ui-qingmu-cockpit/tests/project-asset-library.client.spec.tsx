...
it('filters real catalog entries and opens image or voice previews without a write action', async () => {
  ...
  fireEvent.click(await screen.findByRole('button',{ name:'预览林予',exact:true }))
  expect(screen.getByRole('region',{ name:'素材预览' }).querySelector('img')?.getAttribute('src')).toBe('/face.png')
  fireEvent.click(screen.getByRole('button',{ name:'音色',exact:true }))
  expect(screen.queryByRole('button',{ name:'预览林予',exact:true })).toBeNull()
  fireEvent.click(screen.getByRole('button',{ name:'预览林予音色',exact:true }))
...
it('aborts the old project read and cannot display its late result in a new project', async () => {
  ...
  await screen.findByRole('button',{ name:'预览林予音色',exact:true })
  finish(page([picture]))
  await waitFor(()=>{expect(screen.queryByRole('button',{ name:'预览林予',exact:true })).toBeNull()})
})
