...
it('saves and restores explicit frame roles, blocks mixed mode without dropping references', async () => {
  const { port, view } = mount()
  await chooseAll()
  fireEvent.change(screen.getByRole('combobox', { name: '图1用途' }), { target: { value: 'first_frame' } })
  expect(screen.getByRole('button', { name: '保存引用草稿' }).hasAttribute('disabled')).toBe(true)
  expect(screen.getByRole('button', { name: '预览实际请求' }).hasAttribute('disabled')).toBe(true)
  expect(screen.getByRole('button', { name: '移除音色' })).toBeTruthy()
  fireEvent.change(screen.getByRole('combobox', { name: '图2用途' }), { target: { value: 'last_frame' } })
  fireEvent.click(screen.getByRole('button', { name: '移除音色' }))
  fireEvent.click(screen.getByRole('button', { name: '保存引用草稿' }))
  await screen.findByText('已保存草稿版本 1。')
  const saved = port.saveReferenceVideoDraft.mock.calls[0]![0].request
  expect(saved.bindings.map(item => item.frameRole)).toEqual(['first_frame', 'last_frame'])
  view.unmount()
  render(<ReferenceVideoWorkspace projectId="p" frameId="f" initialPrompt="" port={port} />)
  fireEvent.click(screen.getByText('精确引用 · 导演稿与候选'))
  await screen.findByText('已载入草稿版本 1。')
  expect(screen.getByRole('combobox', { name: '图1用途' })).toHaveProperty('value', 'first_frame')
  expect(screen.getByRole('combobox', { name: '图2用途' })).toHaveProperty('value', 'last_frame')
  fireEvent.click(screen.getByRole('button', { name: '预览实际请求' }))
  await waitFor(() => expect(port.referenceVideoPreview).toHaveBeenCalled())
  expect(port.referenceVideoPreview.mock.calls[0]![0]).toMatchObject(saved)
  expect(port.queueReferenceVideo).not.toHaveBeenCalled()
