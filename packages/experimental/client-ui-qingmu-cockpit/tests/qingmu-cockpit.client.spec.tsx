...
function makePort(overrides: Partial<QingmuYimengPort> = {}): QingmuYimengPort {
  const base: QingmuYimengPort = {
    setAssetLibraryState: vi.fn(async () => { throw new Error('Dedicated asset library fixture') }),
    ...
    readLocalVideoSource: vi.fn(async () => {
      throw new Error('Local video source status is not part of this fixture')
    }),
    registerLocalVideoSource: vi.fn(async () => {
      throw new Error('Local video source registration is not part of this fixture')
    }),
    recoverLocalVideoSource: vi.fn(async () => {
      throw new Error('Local video source recovery is not part of this fixture')
    }),
    ...
    recoverLocalVoiceCandidate: vi.fn(async () => {
      throw new Error('Local voice recovery is not part of this fixture')
    }),
    readLocalVoiceCandidateContent: vi.fn(async () => {
      throw new Error('Local voice content is not part of this fixture')
    }),
...
describe('embedded Qingmu entry scope', () => {
  ...
  it('occupies the application instead of opening a cockpit over the conversation', async () => {
    ...
    fireEvent.click(within(navigation).getByRole('button', { name: /生成与审看/ }))
    expect(screen.getByRole('navigation', { name: '创作流程' })).toBe(navigation)
    const batch = screen.getByLabelText('整集批量生成')
    fireEvent.click(within(navigation).getByRole('button', { name: /01故事/ }))
    fireEvent.click(within(navigation).getByRole('button', { name: /生成与审看/ }))
    expect(screen.getByLabelText('整集批量生成')).toBe(batch)
  ...
  it('treats a missing storyboard revision as a planning state and clears the prior projection', async () => {
    const workflow = vi.fn()
      ...
      .mockRejectedValueOnce(new Error('storyboard_revision_missing'))
    mount(makePort({ workflow }), undefined, true)
    await waitFor(() => { expect(workflow).toHaveBeenCalledTimes(1) })
    expect(screen.queryByText('分镜尚待规划')).toBeNull()
    fireEvent.click(screen.getByRole('button', { name: '刷新页面' }))
    expect(await screen.findByRole('status', { name: '分镜待规划' })).toBeTruthy()
    expect(screen.getAllByText('分镜尚待规划')).not.toHaveLength(0)
    expect(screen.queryByText('当前项目暂时无法更新。已有素材保留，请刷新重试。')).toBeNull()
    expect(screen.getByRole('button', { name: '前往故事与剧本' })).toBeTruthy()
    expect(screen.getByRole('button', { name: '打开分镜与导演' })).toBeTruthy()
    expect(workflow).toHaveBeenCalledTimes(2)
  })

  it('opens the existing person and scene archive from the asset library without adopting a reference', async () => {
    const port = makePort()
    mount(port, undefined, true)
    await screen.findByRole('heading', { name: '故事与剧本' })
    fireEvent.click(within(screen.getByRole('navigation', { name: '创作流程' })).getByRole('button', { name: /准备素材与分镜/ }))
    const openUpload = await screen.findByRole('button', { name: '上传人物/场景参考' })
    const archive = screen.getByRole('group', { name: '人物与场景参考上传' })
    expect(archive.querySelector('details')?.open).toBe(false)
    fireEvent.click(openUpload)
    await waitFor(() => { expect(archive.querySelector('details')?.open).toBe(true) })
    await waitFor(() => { expect(document.activeElement).toBe(archive) })
    expect(port.uploadLocalReferenceCandidate).not.toHaveBeenCalled()
    expect(port.createHumanDecision).not.toHaveBeenCalled()
  })
...
describe('QingmuCockpit journey', () => {
  ...
  it('shows Host-only token recovery when protected reads fail before projection', async () => {
    ...
    expect(alert.textContent).toContain('原生服务会自动续期')
    expect(alert.textContent).toContain('避免重复提交')
    expect(alert.textContent).not.toContain('qingmu-local.py login')
    expect(episodes).not.toHaveBeenCalled()
    expect(workflow).not.toHaveBeenCalled()
  })
  ...
  it('keeps independent evidence reads available when the workflow projection is invalid', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: zh.trigger }))
    const dialog = await screen.findByRole('dialog', { name: zh.title })
    await screen.findByText('分镜尚未建立')
    fireEvent.click(within(dialog).getByRole('tab', { name: zh.tabGeneration }))
