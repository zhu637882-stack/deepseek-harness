...
When adopting an asset redesign, omitted image model, prompt rewriting, framing and ordered reference settings retain the current card values. Explicit null/default, false or empty references still reset those choices. Matching uses the entity kind and ID, or a unique unchanged name when the ID is omitted; another entity does not inherit its references. The native designer receives the current image model catalog and preserves existing selections unless the requested design needs an explained change.
