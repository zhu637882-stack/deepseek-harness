...
Each record's existing details include read-only rework preparation, not a formal route. The current validated method supplies an exact owner match and its scope, and its rule hash is compared with the original record without replacing it. Current, historical, and unverifiable media bindings remain distinct. Stage instances, unit instances in a sealed plan, approval-lock instances, and an independent formal decision are marked as not provided by the current read contract; this does not prove that they do not exist. A changed lock is not inferred from the owner. Opening these details creates no request, recovery marker, or execution action.

These details also consume the already validated unit feed, without another request. A scope binding is shown only when project, episode, frame ID, display number, frame-content SHA, and storyboard revision match the original Finding. Current and historical bindings remain distinct. This evidence does not supply the still-missing sealed-plan unit instance or approval evidence.
