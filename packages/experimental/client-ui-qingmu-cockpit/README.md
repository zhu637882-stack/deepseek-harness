...
For an actual actor, environment, or prop already shown in that workbench, the user can choose one local PNG/JPEG/WebP and save it as a project-and-element-scoped candidate. The panel previews only Host-reread bytes and displays the input/materialized SHA, source as unverified, rights as not recorded, and the explicit `Unselected · unapproved` boundary. The original request is retained through disconnect or reload; an unknown result offers only receipt recovery and is never silently resubmitted. This entry point does not select a reference, approve content, record formal rights, create PromptIR, or start generation.
