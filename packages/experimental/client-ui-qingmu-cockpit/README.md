...
- The current bounded slices cover `episode_script`, actor/scene/prop `element_profile`, and reference selection or regeneration intent through propose → preview → explicit commit. PromptIR editing, rights exceptions, comments and review decisions, paid generation, automatic selection, approval, signoff, and release controls remain absent. This does not declare Phase 3 complete.
