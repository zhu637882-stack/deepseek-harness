...
For an actual actor, environment, or prop already shown in that workbench, the user can choose one local PNG/JPEG/WebP and save it as a project-and-element-scoped candidate. The panel previews only Host-reread bytes and displays the input/materialized SHA, unverified source, and the explicit `Unselected · unapproved` boundary. Through the existing IMAGO method projection, preview, explicit confirmation, and Yimeng rights ChangeSet, the user may save a structured knowledge record. The saved view still shows the unverified source, every unknown field, and the unselected/unapproved state, and explicitly says record existence is not verified legal fact. The original request is retained through disconnect or reload; an unknown result offers only receipt recovery and is never silently resubmitted. This entry point does not select a reference, approve content, create PromptIR, or start generation.
