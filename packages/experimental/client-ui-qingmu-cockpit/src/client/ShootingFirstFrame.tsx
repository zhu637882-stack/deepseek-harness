...
async function request(path: string, body?: object, signal?: AbortSignal): Promise<unknown> {
  ...
  if (!response.ok) throw new Error(typeof value.detail === 'string' ? value.detail : '本镜请求未完成，请查看开发日志')
  return value
}
...
export function ShootingFirstFrame({ scope, onCommitted }: {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (value.candidate || ['Failed', 'Cancelled', 'Succeeded'].includes(value.task?.kernel_status ?? '')) return
      } catch (cause) { if (!controller.signal.aborted) setError(String(cause)) }
  ...
  return <section aria-label="首帧生成" style={{ width: '100%', height: '100%', overflow: 'auto', padding: '16px', boxSizing: 'border-box' }}>
    ...
    <details><summary>开发日志</summary>
      ...
      <pre style={{ whiteSpace: 'pre-wrap' }}>{JSON.stringify({ preflightId: preview?.preflightId, payloadHash: preview?.payloadHash, estimatedCny: preview?.estimatedCny, blockers: preview?.blockers, requestId, attempt, error }, null, 2)}</pre>
    </details>
  </section>
}
