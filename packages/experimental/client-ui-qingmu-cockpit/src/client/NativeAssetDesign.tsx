...
export function NativeAssetDesign({ projectId, episodeId, port, storyPort, onGenerated, libraryRefreshToken = 0 }: {
  ...
  const [feedback, setFeedback] = useState({ key: changesKey, text: '' })
  const sourceFeedback = feedback.key === changesKey ? feedback.text : ''
  const receiveFeedback = useCallback((text: string) => { setFeedback({ key: changesKey, text }) }, [changesKey])
  ...
  return <section className={css.composer} aria-label="角色与场景生成">
    ...
    {state?.projectId === projectId && state.episodeId === episodeId
      && <AssetSourceFeedback key={`${projectId}:${episodeId}`} state={state} disabled={busy} onContext={receiveFeedback} />}
    ...
    {state && storyPort && <NativeStoryComposer port={storyPort} projectId={projectId} episodeId={episodeId}
      ...
      purpose={{ key: 'asset-design', jsonOutput: true, freshRevision: true,
        sourceKey: JSON.stringify({ stateSha256: state.stateSha256, design, changes, sourceFeedback }), title: '让青木设计素材', description: '导演会结合剧本和素材方法提出完整设计，你可以逐项调整。',
