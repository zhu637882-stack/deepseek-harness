...
import { firstFrameDirectorPrompt, generationContextGuidance, referenceSelectionGuidance } from './director-generation-guidance.ts'
...
export function NativeDirectorComposer({ port, sessionId, scopeKey, ready, target, onCommitted, mode = 'shot' }: {
  ...
  return <section className={css.composer} aria-label={mode === 'cut-sound' ? '向整片声音导演提要求' : '向当前镜头的导演提要求'}>
    ...
    <button type="button" disabled={busy || !!text.trim()} onClick={() => {
      const next = '请整理当前镜头的视频生成稿。先读取已保存的完整导演设计、剧本、全片风格、前后镜接续依据、引用草稿和实际素材。按 Leos 六部门与本片方法核对叙事、空间、动作时序、运镜、表演、逐句对白语气、光影和声音。读取完整设计作为依据，按共用 executionPromptGuidance 整理实际可拍的执行稿，使用 executionPrompt；不要把整份设计文档或检查反馈原样送进生成提示词。准确说明各引用素材的用途与接续关系。有可核实的冲突才修正原设计，说明依据；估计坐标下的一次机位计算不能证明原镜头不可行，不因此删除导演安排的构图、动作或人物。画面设计与起始图描述均需改变时，用 qingmu_save_director_plan 同步提交 directorPlan.visual 与 imagePromptCn，再读取新来源编译，不能追加另一套文字覆盖旧设计。保留导演安排的换场、时间跳跃、多人对白、画面文字与动作变化，不把局部终态扩大为全程要求。核对声音的制作环节：已决定在剪辑后添加的整片配乐、跨段声桥写入 editorialContext；soundPlan 保留本段原生对白、呼吸、环境与拟音，并说明该后期配乐不在分段视频中生成。剧中有源音乐、歌唱及导演明确选择的原生配乐不能一并删除。整理旧稿时完整保留其配乐意图，不能仅删去音乐段落。保持未要求修改的台词、素材和生成参数。选择前镜引用前，用 qingmu_read_reference_video_candidates 读取候选、已有原生音画观察与时间码评论，按视频 SHA 和版本区分问题属于哪个结果；读取相关 reviewPage 和评论页。先将独立观察和听写与报告 comparisonIntent 中的生成原要求比较，再核对现设计；没有评论不代表没有审片。评论是已记录的观察，不等于审片通过或本次已经看图；未知或有问题的终态不能默认为动作已经完成。再用实际抽帧与看图工具核对；仅有素材信息时明确尚未看过，不能声称观察了画面。保存后读取回执并预览实际请求，报告具体调整与尚未验证处。不要提交视频、选用候选或修改其他镜头。' + '\n' + executionPromptGuidance + '\n' + generationContextGuidance + '\n' + referenceSelectionGuidance
