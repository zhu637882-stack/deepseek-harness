...
import { clearTakeVersionSelectionMarker, createTakeVersionSelectionMarker, writeTakeVersionSelectionMarker } from './take-version-recovery.ts'
...
export function ShootingReviewWorkspace({ projectName, episodeName, projectId, episodeId, projection, selectedShotId,
  onSelectShotId, onNavigate, directorAssistant, port, t, testState }: Props) {
  ...
  const versions = stack?.subject.versions ?? []; const browsed = versions.find(version => version.takeId === browseId)
  const state = testState ?? statusOf(stack, browsed); const primary = usable(browsed) && stack?.capabilities.canSelect === true && !browsed.isSelected && load === 'ready'
