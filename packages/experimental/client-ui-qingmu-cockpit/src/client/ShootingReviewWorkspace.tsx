...
export function ShootingReviewWorkspace({ projectName, headerActions, hideHeader, episodeName, projectId, episodeId,
  ...
  return <section className={css.workspace} aria-label="拍摄与审看">
    ...
    <div className={css.grid}>
      ...
      <aside className={css.inspector}><div className={css.switcher}><button type="button" aria-pressed={panel === 'requirements'} onClick={() => setPanel('requirements')}>当前要求</button><button type="button" aria-label="原生导演助手" aria-pressed={panel === 'assistant'} onClick={() => setPanel('assistant')}>导演助手</button></div>{panel === 'requirements' ? <div className={css.requirements}><h2>镜 {String(current.frameNo).padStart(2, '0')} · 当前要求</h2><h3>对白</h3><p className={css.dialogue}>{dialogue.join(' / ') || '本镜暂无对白'}</p><AutomaticFrameRequirementsEditor key={`${projectId}:${episodeId}:${current.shotId}`} projectId={projectId} episodeId={episodeId} shotId={current.shotId} port={port} onCommitted={onCommitted} onRequirementStatusChange={onRequirementStatusChange} onReturnToStoryboard={returnToStoryboard} onSelectShot={onSelectShotId} /></div> : directorAssistant}</aside>
    </div>
  </section>
}
