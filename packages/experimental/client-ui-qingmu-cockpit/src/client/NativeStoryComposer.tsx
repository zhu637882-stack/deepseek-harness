...
export function NativeStoryComposer({ port, projectId, episodeId, source, settings, disabled, onAdopt, purpose }: {
  ...
  async function send(revise = false) {
    ...
    try {
      ...
      await port.send(current.sessionId, `本次工作阶段：${purpose?.title ?? '整集编剧'}。入口已提供本次来源，输出供本页采用的候选正文；不是已有镜头修改，不要求镜头绑定，不调用镜头写入工具。需要核对当前已保存素材或参考图时，用 qingmu_read_asset_design 读取本集设计和本项目图片目录，再用 qingmu_view_reference_image 查看相关图像。已有图片的空间、结构与设计文字冲突时，明确可见事实和不可见部分，协调本次设计，不凭文字声称图像已遵守。图片引用需保留实际 assetId/SHA 和具体用途。只有完成本页采用与保存后才成为项目正式内容。\n${prompt}${revision}`,
