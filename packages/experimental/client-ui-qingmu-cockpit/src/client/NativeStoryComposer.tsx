...
export function NativeStoryComposer({
  ...
  async function send(revise = false) {
    ...
    try {
      ...
      const prompt = purpose?.prompt ?? `为青木当前项目写出完整可拍摄的中文短剧。项目：${projectId}，剧集：${episodeId}。本次文字是当前创作依据，不读取其他项目或镜头。\n创作设定：${settings}\n创作原点或现有稿：\n${source}\n请实际读取 cinematic-director、open-film-writer 及其必要写作参考，先理解人物目标、因果与关系变化，再完成自然对白和可见行动；不要只返回大纲或计划。完整剧本单独放在一个 txt 代码块，格式为“场景一：地点·时间”“动作：具体动作”“姓名：台词”，每项各占一行。理解剧情不可缺少的年代、人物关系和世界例外必须落实到正文中可见或可听的事实；人物表、导演阐述和时长估算放在代码块外。交稿前从正文检查场景的室内外、门两侧、人物进出、关键道具的持有与连接状态，以及每个行动的前提和结果；台词所指动作必须在当前地点成立，不能用未交代的换场补漏洞。广告短剧让产品因人物目标和剧情发展被使用，继承上传产品信息和用户提供的规格，未证实的功效不自行补写。发现矛盾先修候选正文，允许有据可循的省略剪辑。遵守本项目已选风格和时长。新构思由你提出供用户采用；不要改动现有正式剧本或媒体，不声称用户已审看。`
