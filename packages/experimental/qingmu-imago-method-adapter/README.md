...
`src/director-assets/registry.ts` is the single admission registry for vendored third-party director assets under `assets/director/<repo-id>/<full-commit>/`. Each package carries its upstream `LICENSE`, a per-file `PROVENANCE.sha256` ledger, and an `ADMISSION.md` stating source, scope, blockers honored, and usage bounds. `verifyAllDirectorAssets` fails closed on any unlisted file, malformed provenance ledger, or byte mismatch. Admission covers `director-skill-core` (MIT, content), `storyboard-skill` (MIT, pure modules), `jellyfish` (Apache-2.0, pure modules), `nautilus-studio` (Apache-2.0, pure modules), `ai-visual-director` (MIT, content), `director-skills-travel` (MIT, content), `arcreel` (skills/ subdir MIT only), and `bluefish` (Apache-2.0, pure module; silent-placeholder blocker recorded).

`src/director-assets/assembly.ts` adds the static stage assembly: `DIRECTOR_STAGE_CARDS` binds every provenanced file (except LICENSE) to explicit IMAGO stage ids and a card kind (`method_card` / `checklist` / `failure_codes` / `eval_set` / `tool_module`); `directorStageCards(stageId)` returns the cards a stage may load (unknown stages return an empty set, never a guess); `loadDirectorAssetFile` serves content only after verifying that file's provenance line. Assembly is guidance-only: cards supply text and module references, no runtime execution and no state writes. Work-order-driven activation is a separate, later concern.
