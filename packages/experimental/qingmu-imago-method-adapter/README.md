...
The [stage-cards side band](src/director-stage-cards.ts) serves that assembly over the method handler as two strictly validated endpoints: `directorStageCardsMethod` (`{stageId}` → card list) and `directorStageCardMethod` (`{repoId, path}` → provenance-verified content). Responses carry a guidance-only authority block (zero provider calls, zero cost, no selection or decision semantics); unregistered or tampered cards fail closed. These endpoints still do not touch work orders, skills, or executable tools — weaving cards into method projections stays deferred to the PromptIR field-mapping slice.
