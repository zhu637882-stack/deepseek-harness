...
import { DirectorAssetRegistryError, directorAssetEntry, verifyDirectorAssetPackage } from './registry.ts'
...
/** Static candidate file-to-stage mapping, not a legal work-order grant. */
...
/** Candidate coverage only; none of these rows connects to a runtime loader. */
...
/**
 * List candidate cards in registry order without granting work-order activation.
 * @param stageId - exact uppercase IMAGO stage ID.
 * @returns matching static candidates, or an empty list for unknown stages.
 */
...
/**
 * Read inert source text after verifying the complete package and pinned ledger.
 * This is not a sandbox: callers must own a stable local assets root during the read.
 * @param assetsRoot - trusted local assets directory.
 * @param repoId - exact registry repository ID.
 * @param relPath - admitted, provenanced file path; never evaluated as code or instructions.
 * @returns UTF-8 source text; rejects unlisted paths, ledger errors, or byte drift.
 */
export async function loadDirectorAssetFile(
  ...
  await verifyDirectorAssetPackage(packageDir, entry)
