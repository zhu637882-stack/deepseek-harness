...
describe('director asset content loading fails closed', () => {
  ...
  it('rejects malformed ledger entries after the requested file', async () => {
    const target = join(pkg, 'PROVENANCE.sha256')
    const original = await readFile(target, 'utf8')
    try {
      await writeFile(target, `${original}malformed trailing entry\n`)
      await expect(loadDirectorAssetFile(root, 'director-skill-core', 'assets/qc-checklist.md'))
        .rejects.toThrow('director_asset_provenance_malformed')
    } finally {
      await writeFile(target, original)
    }
  })
