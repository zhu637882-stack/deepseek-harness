...
导演来源可包含 executionSuffix（逐字对白、视觉媒介和画面文字范围）。浏览器可用的 ./reference-prompt 导出供单镜和批量共用的创作说明与装配函数，保留执行描述和引用顺序；最终请求仍由 Writer 校验。

参考目录在 `source` 中保留已报告的所属实体、用途、选用、质量与人审状态。缺失或格式无效的值保持未知。它们用于区分不同候选和旧帧；已选或质量通过不代表当前创意验收。没有增加私有生成配置或操作权限。

原图设计可包含 `spatialReference`：生成时的场景体块、摄影机和构图图片 SHA。它从不可变图片来源投影导演设定，不以当前草稿替代，不暴露授权信息，也不冒充测量几何。后续导演看图时可以与实际生成像素对照。
...
`referenceCandidates` 要求 `jason.qingmu-reference-asset-candidates.v1`、`targetType: element_profile`、请求中的项目、元素类型与目标 ID、非负档案修订号、小写 `elementSnapshotSha256`，以及字面值 `humanApprovalInferred: false`。每个返回候选还绑定资格类型/检查/是否通过/身份、上传回执与当前权利记录 SHA。`provider_formal_consistency` 和 `local_file_integrity` 始终分开；权利 SHA 变化会让本地资格过期，而不是保留选择资格。Host 会先校验候选的项目与 owner 绑定、ID、布尔值、SHA-256 字段，以及准确的状态和决定取值集合，再返回投影。
...
`generationSubmissionEnabled` 是纳入报价校验和的必填布尔值，表示全局运行时付费开关、正数预算及项目/剧集范围，不代表预算预留或用户批准。禁用实例仍返回估算和已有候选，但在写队列前拒绝新任务；已有任务的精确恢复仍可使用。
...
外部原片使用带版本的 Take、验收和审核接口；仅含内部生成视频时保留 v1。来源绑定上传回执、来源登记、视频字节和镜头修订，不宣称平台执行已核验。审核 v2 提供当前来源，因此即使评论对象未变，来源变化也会使旧决定失效。交接 v2 可把已解码的原始 MP4 同时用于画面和内置音轨，仍要求当前 QC 与批准。
...
视频引用使用既有草稿、素材准备、预览和核价操作。图片、音频和视频独立编号，视觉输入可以只有视频。最多五段经检查的 MP4/MOV，合计不超过15秒，输入加请求输出不超过30秒。报价计入输入视频时长，拒绝缺失时长或只计算输出的价格。视频报价校验时将 billableSeconds 投影为六位小数字符串，响应仍为数字；纯图片/音频报价维持原编码。源视频和采用状态保持不变。

图片目录保留原图生成配置中的创作字段：原始取景、站位状态、共用空间、关联场景及实际提交描述。缺失或损坏的历史保持未知，不返回授权字段，也不以当前设计替代历史。这些是生成意图，不是图片实测几何。

图片引用可明确设置 `frameRole` 为 `first_frame` 或 `last_frame`。首帧模式要求恰好一张首帧、最多一张尾帧，不能混入其他参考素材。草稿与报价哈希包含用途，预览核对供应商实际媒体类型并保留图片编号。不设置该字段时，旧请求校验和与多参考行为保持不变。

导演来源可选提供 generationPrompt，与完整研究依据并列的生产正文。适配器校验并保留它，不改写已存提示词。旧来源可以省略该字段，不以复制整段研究上下文代替。原有 20,000 字符请求上限在原生带入完成后仍有效。

音频和视频目录保留记录中的正数 `durationSec`；缺失或无效值保持未知。这些元数据用于提前安排引用，准备和提交时 Writer 仍检查原文件。

已有布局与机位时，原生准备先预览取景，结合实际参考核对画面左右、背景与遮挡。没有人物体块的预览不能证明演员站位；矛盾来源回到现有导演或素材流程整理。
