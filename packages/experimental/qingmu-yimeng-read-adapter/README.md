...
The download contract is GET-only. The Host issues a one-use capability only after an authenticated Yimeng projection reports every shot has a current selected video, exactly one authoritative dialogue-audio asset, exact media SHAs, matching duration, current QC, and natural-person approval evidence. That capability is bound to the project, episode, source SHA, and projection SHA. The Host first spools the full response to a private temporary file, verifies its declared SHA and size, and only then sends package bytes to the browser. Hashed capability bindings and terminal status persist under the private DSH home so refresh or Host restart can recover without a second Writer download. Yimeng also rejects declared media types that do not match actual container bytes, and builds a deterministic ZIP_STORED package with the pinned OpenTimelineIO 0.18.1 `otio_json` adapter. Availability does not imply Aoki production, Yimeng release, or human editorial sign-off. Reads, refreshes, and downloads make zero Provider calls and no business-state mutations.
