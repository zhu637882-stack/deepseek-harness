...
- Rules — write audio as its own sentence naming sources, or expect a score composed under your scene: "Audio: rain on a tin awning, one distant dog, no music." Say "no music" whenever you intend to score in the edit — that is the one licensed negation in a positive prompt on this family, because it lands as an audio instruction rather than as a visual noun. The negative field is `part`: it exists on some API surfaces and not in most consumer ones, so unless you can confirm it, convert every visual exclusion into a positive fact about the frame. Preserve the scripted speakers and verify their voice/mouth ownership using the actual selected model; see [sound-and-dialogue.md](sound-and-dialogue.md).
...
The following motion score is an upstream planning heuristic, not a measured model limit or a Qingmu gate. Its worked examples illustrate possible staging alternatives. They never authorize deleting directed actions, locking the camera, splitting a continuous take or silencing a speaker. Validate the actual route and preserve the script and director design.
...
Upstream example risk bands used **4**, **6** and **8**. These are unverified heuristics, not capability limits; do not reject a plan or alter it because of these numbers.
...
- Why short beats long for character work. Identity error accumulates with frame index, so the earliest seconds are almost always the cleanest frames in a generation. Choose duration from the directed performance and verified current model support, including complete dialogue and contact actions. Generating 10s to use 4 costs more, not less, once you count retries; see [production-workflow.md](production-workflow.md).
...
| "It came out warped / melty" | rescore | inspect the actual defect and preserve directed motion while resolving timing, geometry or route limitations |
