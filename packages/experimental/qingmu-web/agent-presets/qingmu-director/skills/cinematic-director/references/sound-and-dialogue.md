...
`soundPlan` records explanations; the renderer does not execute prose. In particular, writing that a title was removed does not change `directorPlan.frameTextOverlays`. Read and save the actual bound shot director plan to change those overlays, then read the executable field back. When a source clip already contains the required text, distinguish that burned-in text from compositor overlays using the actual image. Do not remove unrelated overlays, claim to erase source pixels, or report an unsupported edit as completed. If the relevant shot is outside the current tool scope, report that limitation instead of saving a fictional result in the sound notes.
