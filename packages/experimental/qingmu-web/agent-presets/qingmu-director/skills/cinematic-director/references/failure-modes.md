...
| Re-stage | Propose clearer timing or geometry while retaining the director's action and camera intent. Simplification is a creative decision, not an automatic repair. | F4, F6, F13 |
...
- Fix L1: verify the exact scripted text, each speaker's voice and mouth ownership, intended delivery and timing against the selected model. Preserve multi-speaker turns and full lines; [sound-and-dialogue.md](sound-and-dialogue.md) governs execution alternatives.
- Optional director-approved coverage change: hide the mouth. Three-quarters away, over-the-shoulder, hand at the face, or a cutaway to the listener — then the line is VO in the edit and always syncs.
- Fix L5: design the scene so the important lines land on listener shots. Reaction coverage is free insurance.
- Fix L6: evaluate the delivered native audio against the sound plan. Preserve satisfactory audio; replace or repair only the diagnosed defect with continuity maintained.
...
| F6 | 7 Shot list & coverage | Explicit timed camera choreography with geometry and motivation, including compound moves where directed |
