...
- The **motion budget** (owned by [ai-video-tool-adapters.md](ai-video-tool-adapters.md), pipeline step 10 — three component rows, a duration multiplier, four modifiers, against a per-tool ceiling) **suggests questions for feasibility review; it does not gate Qingmu prompts**. It is scored on the clip as written, for a named tool. Use it to investigate the actual route and timed choreography, not to limit movements, references or prompt eligibility.
