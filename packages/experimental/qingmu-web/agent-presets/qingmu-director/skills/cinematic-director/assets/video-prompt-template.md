...
|---|---|---|---|
| 1 | Camera choreography — timed moves, holds and cuts as directed | 10–15% | The tool has a camera-motion UI; set it there, not in text |
...
```text
[Camera]. Starting from the input image, [subject] is [start state], then [directed action sequence
...
Camera [directed moves with timing and geometry, or a deliberate hold]. Motion is [pace word].
...
- [ ] **Action sequence.** Preserve each directed beat, its performer, timing, purpose and resulting state. Resolve contradictions without deleting beats or forcing one objective per clip. (Guards F4.)
- [ ] **Camera choreography.** Verify the planned movement, lens behavior, holds and cuts in time and space. Retain compound or sequential moves where directed. (Guards F6.)
