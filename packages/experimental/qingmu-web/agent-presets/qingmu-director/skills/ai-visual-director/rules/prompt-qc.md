...
□ 当前导演明确不需要的制作标注是否误入画面？剧情文字与镜内界面是否按设计保留？
...
**青木来源**：当前项目保存的剧本、画风和导演设计；包内 `state/format-contract-state.md` 只是空白结构参考，不能作为当前作品事实。模板画幅、密度和镜数只是示例。

### 维度 2：正负描述与当前设计一致

对照 `rules/negative-prompt.md` 按需选择；不要求出现通用负面词，不按风格、关系、年代或平台自动拼表。

- 有无否定已选水彩、动漫、胶片、写实等画风的词？
- 有无删除剧情文字、世界例外、当前动作、肤质或空间特征的词？
- 有无为解决参考卡污染而误删剧中真实物件、招牌或界面？
- 所选模型是否支持该参数？没有适用负面项时允许省略。
...
□ 构图描述是否与画幅匹配？（横摇、俯仰和复合运动依具体取景设计判断，竖屏不构成运镜禁令）
```
...
| **返修** | 意外制作标注污染目标画面 | 修复非导演要求的标注，保留剧情文字与镜内界面 |
| **阻断** | 海报 @图进入视频 prompt | 阻断输出，移除海报 @图 |
| **返修** | 正负描述与导演设计矛盾 | 移除冲突的负面词，保留创作意图 |
