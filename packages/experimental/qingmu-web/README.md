...
This method update explicitly carries reviewed predecessor references to the current content hash, including the immediately previous release. Frozen project contracts and existing media stay unchanged; undeclared future changes still require compatibility review.
...
The preset mounts the stock `compaction-basic` engine in an isolated session group, using the host token meter. For `deepseek-official/deepseek-flash`, automatic summarization starts at 18% of context capacity (180,000 tokens at its configured million-token capacity), with a 32,768-token recent-tail budget. Other model routes retain the 65% threshold. These are pressure and retention targets, not hard request limits: complete indivisible units and current source reads can exceed them. Compaction preserves the durable conversation log; saved Writer designs and drafts are unchanged. The director rereads complete current creative sources before production. No tool-result pruning is mounted. A failed summary remains a reported runtime error, not a successful recovery.
...
Only `qingmu-director` sessions receive the stable Chinese persona in [agent.cordis.yml](agent-presets/qingmu-director/agent.cordis.yml) and scoped native tool schemas. The persona uses actual Writer context and the bundled cinematic-director and ai-visual-director methods, reading linked references through the scoped resource reader. Script and director decisions govern dialogue, performance and camera choreography; upstream numeric heuristics are advisory. Existing-shot edits read and save the complete director design, then reconcile the saved reference draft before preparing materials and previewing the actual request. Production prompts retain specific performance, camera, action and sound decisions; concise progress summaries do not justify summarizing those decisions away. The preset checks content as well as source identity, reconciles reference roles and preserves story-relevant visible text. It checks scene prerequisites before staging, uses existing shots as batch continuity context, and distinguishes visible cast, reverse-angle backgrounds and the unwanted surroundings of prop images. Conflicts return to their owning source instead of being hidden by a new location or actor. It preserves dialogue verbatim; explicit canonical dialogue replacements use the existing ChangeSet tools before reconciling the reference draft. Source changes require rereading context and methods rather than rebinding old text to a new hash. PromptIR field suggestions remain available for explicit field edits or shots without a reference draft; first drafts also require that no Draft or Ready already exists. Saving a working draft does not generate or approve media. Its exact model input is pinned through a keyless native-loop snapshot.
...
The bundled prop method defaults movable-object identity images to one object in one state on a seamless solid background. Scale and interaction notes remain in the design; an interaction demonstration is a separate requested image. Fixed architectural elements retain necessary installation context. These instructions do not clean existing images or prove generated pixels obey them.

The native `production-design` supplement bundles two MIT-licensed director-skills methods at revision 35ca0a2b4cd55b0668aa9ea0f40273324b774bb4. Asset authoring loads it through the ordinary native skill and pinned resource tools; it maps art direction and compact continuity anchors to existing Qingmu fields.
