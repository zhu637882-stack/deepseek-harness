...
首帧准备的 HTTP 409 保留有长度边界、已脱敏的校验原因，让导演区分输入长度、引用及场景错误。适配器不自动重试，也不暴露其他服务器诊断。
...
项目复制命令可预览校验归属的来源快照、提交精确复制请求并恢复回执。适配器核对来源标识、请求哈希及未调用生成的标记。Writer 负责标识映射、来源冲突检查和事务回执重放。

同源原生视频审片桥沿用浏览器或原生身份，将明确的 POST 提交与只读 GET 恢复转交 Writer。Writer 核对分集归属与候选文件，保留生成时绑定的提示词，并复用等价审片任务。卸载时移除桥接路由。
...
`updateProject` 通过既有、校验项目归属的 Writer PATCH 接口保存项目名称与归档状态。输入只允许项目标识、名称与状态，返回值必须匹配项目及所提交的字段；此操作不生成媒体、不修改创作设定；刷新后的项目列表读取会恢复已保存状态。
...
`/api/qingmu/entity-draft-human-review/decision` 路由针对精确当前的 PromptIR 关联草稿接收一项明确的 `accepted` 或 `rejected` 决定。这个直接同源浏览器路由拒绝 Bearer 凭据，只转发 Writer 签发的 `jason_token` cookie。它先取得短时 Writer 意图证明，再把未改变的四字段决定发送一次；actor、审核人、自然人和 session 身份绝不来自浏览器输入。网络或 5xx 结果不明确时，用原始幂等键和请求 SHA 进行纯 GET 恢复。Writer 原子地重核 Ready PromptIR、分镜帧、草稿内容、元素 profile、已选参考、参考包、自然人绑定和浏览器会话。结果不产生任务修改、outbox 事件、Provider 调用、媒体批准、提示词批准或签收。参见[决策记录](../../../.agents/notes/implemented/feature/2026-09-02-qingmu-prompt-ir-entity-draft-human-review.zh.md)。

`queueProductionTake` 只接受准确项目、剧集、故事板、镜头、Take 类型/编号、人工首帧选择回执、当前视频报价及其服务端原文确认文字，以及明确 Ready 确认。Host 自行重读当前 Ready PromptIR、首帧报价与视频报价，运行当前 PromptIR 方法，验证 HMAC 证明以及 D/E 字段、合同、卡片、来源、提示绑定，再组装 Writer 请求。浏览器不能提供 owner、Ready、选择、批准、force、Provider、模型或路由权威。Host 按镜头与 Take 编号派生确定性幂等键；丢响应、刷新或 Host 重启后都重放同一语义命令，并且只接受匹配的 Writer 回执。界面只开放 Take 1 `initial` 与 Take 2 `targeted_rework`；伪造的 Take 3 意图仍会到达 Writer 的持久上限，并由 Writer 拒绝。此桥不调用 Provider、不启动 Worker，也不把排队写成已经生成。参见[决策记录](../../../.agents/notes/implemented/feature/2026-09-02-qingmu-production-take-bridge.zh.md)。

[本地参考候选操作](src/local-reference-candidate.ts)可列出、上传、恢复、读取或检查一个实际存在的人物、环境或道具图片候选。Host 只接受一张不超过 3 MiB 的严格 PNG/JPEG/WebP，计算真实字节 SHA-256；上传或资格结果未知时只用 GET 查询原回执。易梦拥有鉴权实体范围、图片解码校验、原子本地落盘、canonical `assets` 记录、ChangeSet/outbox 和回执。唯一权威 rights ChangeSet 保存知识记录后，用户明确触发的本地资格检查会重新读取上传回执、落盘字节、权利 SHA、修订和快照。它只记录 `local_file_integrity`；权利与正式一致性仍未核验，Provider 调用保持为零，资产继续保持 `Unselected`、未批准，直至进入既有明确选择预览与提交。参见[决策记录](../../../.agents/notes/implemented/feature/2026-08-31-qingmu-local-reference-qualification.zh.md)。

[场景规划操作](src/scene-planning.ts)读取、保存或恢复一场导入剧本的规划结构。Host 绑定剧本版本/完整 SHA、分镜版本/SHA 和准确请求摘要。易梦原子保存文本实体、镜头、真实结构快照及既有 ChangeSet/outbox/回执记录。恢复返回原 ID，不再次 POST。只有分镜不存在时可初始化；已有规划镜头使用 canonical Edit 修改。嵌入式驾驶舱完成 canonical 重读后，发送一条限定准确来源和范围的保存通知；外层 Writer 负责校验、重读 canonical 场景与导演上下文、刷新工作流、去重及准确定位场景/镜头。通知缺失、畸形、陈旧或范围不符时，外层均不显示成功。不创建提示词、参考媒体、批准或制作阶段。[决策记录](../../../.agents/notes/implemented/feature/2026-08-29-qingmu-scene-entry.zh.md)解释结构 Ready 的区别。

项目初始化与文本导入使用[创作约定](src/creation.ts)中的窄操作。初始化前，只读的创建目录操作会将 Writer 的源文本/导演方法目录与基础视觉风格、风格包目录合并。浏览器必须明确选择目录中提供的一项基础画风、风格包和可用导演方法；适配器既不写入默认选择，也不修补旧的待提交载荷。易梦在既有事务日志中原子创建项目、第一季、首集及按操作者作用域持久保存的回执。恢复查询原幂等键与请求摘要；同键异参会失败。UTF-8 TXT 限制为 128 KiB、64000 字及 1000 行，SHA-256 取自真实输入字节。TextImportService 拥有解析校正、指纹、剧本版本和确认记录。读取不创建或修复草稿；只有来源和前序索引仍为当前状态时，显式同键重试才可补齐未完成创建。Host 绝不自动重试写入。确认只保存解析文本，不启动阶段、不生成资产、不提升 Ready、不选择 Take、不批准媒体。

适配器自身不写数据库。它校验浏览器输入，只从 Host 环境获取 `YIMENG_API_TOKEN`，并把命令转发给易梦拥有的 HTTP API。项目归属、修订检查、持久 ChangeSet、幂等回执、outbox 事件和下游失效仍由易梦掌权。

导演 Provider 执行 seam 仅属于 Host，且默认禁用。隔离部署可以选择不含凭据的 loopback mock transport，或单独校验的 production-capable transport，二者不能同时启用。production-capable 配置固定使用 `deepseek-official`、`deepseek-v4-pro`、`https://api.deepseek.com/chat/completions`、关闭推理、不使用工具或文件、`maxRetries: 0`，并限制输入／输出 token；只有私有配置显式启用一个已签发任务及其方法绑定后，它才会运行。禁用状态仍可签发不可变的易梦工作单，并为提交前锁读取其私有 binding，但不会领取 dispatch、创建预留或 submission outbox、挂载或读取外部凭据文件，也不会调用 transport。Writer 在 prompt 与工作单中嵌入同一份完整、内容寻址的 `qingmu.director-proposal.v1` JSON 合同；Host 消费这份已签合同，不维护第二份手写 schema。合同要求只输出一个 JSON 对象，禁止 Markdown 与解释文本，并精确约束根字段、条目字段和范围。文本范围按 Unicode code point 计数，组合字符中的各 code point 分别计数。已签占位符策略只做 ASCII 首尾空白裁剪与 ASCII 小写转换，随后拒绝 `unknown`、`未知`、`不确定`、`n/a`、`tbd`、`null` 和 `none`；没有建议时省略该条目。launcher 的 `director-submit-once` 命令是唯一激活入口：默认预检绑定公开锁 SHA、私有 binding、输出合同 SHA、Writer immutable 核验、精确零尝试账本状态、从权威 IMAGO 字节重新物化的方法包，以及未消费的 DSh 凭据解析；精确 production 确认值随后会在任何自有进程启动前创建持久 no-replay 审计，且绝不修改 `instance.json`。冷备与恢复包含该尝试审计，因此恢复不能重新武装已被 fence 的任务；自有进程停止后会移除瞬时 override。保守输入上界使用 UTF-8 提示词字节数作为 token 数上界，写入工作单，并在 `prepareCall` 前再次校验。transport 启用后，Host 保持既有 binding → prepare → transport → complete/unknown 顺序，通过一个 `ctx.llm.prepareCall()` handle 和严格一次 `prepared.stream()` 执行。成功结果必须携带稳定的真实 completion id 与 finish reason、token/cache 用量及输入输出谱系；响应 request id 只在 Provider 实际返回时保留，保持可空且绝不伪造。Provider 完整响应后，Host 会先对准确 UTF-8 输出计算哈希并保留 completion id、request id、finish reason 与 usage，再解析提案；schema 非法的提案以 `provider_response_invalid` 终结，只保留这些有界事实，绝不保存原始输出。未取得完整事实的 transport 故障仍属于独立的 `submission_unknown`。两种状态都持久化且绝不重试。该 seam 不向浏览器 RPC 暴露执行密钥、claim、permit、Provider payload、凭据或模型原文，也不创建第二份任务或费用账本。
...
既有 Take 选择命令可把已登记当前来源且可播放的本地视频选入审看。选择保留待检查质量状态，不生成生产血缘或批准。QC 与生命周期命令接收准确的外部验收对象。来源或媒体变化会拒绝新命令；结果不明时仍查询原回执。

基础画风预览使用同源只读 `/api/qingmu/creation-style-preview?styleId=…` 路由。它只接收严格校验的目录 ID，读取固定 Writer WebP 路径，拒绝重定向并检查媒体类型和大小。只有存在且路径匹配的目录图片才返回 Host 预览地址；浏览器凭据不转发到上游。

原生素材卡片区分剧本事实、导演推导和世界设定例外。每张图保留相关设计依据、视角，以及最多九张按顺序绑定版本哈希和用途的项目参考；可选框选使用原图像素。万相编辑通过现有任务队列接收这些图片及描述，来源变化使报价失效。修改描述保留已选图片并提示比较，生成候选不替换原图。导演写定的场景视角和所需文字保持原意，过长请求明确失败而不截断。参考预览和报价不上传或生成媒体。 每张卡片可以单独选择素材画幅；自动模式下新图沿用项目画幅，修改沿用最后一张参考图画幅。报价、模型请求和刷新恢复保留该选择。完整画面模式保留全部设计背景作为来源，图片执行当前画面描述；旧拼接模式仍会加入背景正文。

素材创作在编写图片描述前读取项目基础画风、协调后的风格包和有效导演方法。来源哈希包含这些设定，设定变化会要求重新读取未保存的设计，而不是写完互相冲突的设计后再追加风格。

导入剧本的分镜可按场次继续建立，各场保留原始来源和回执，新增镜头共用本集最新结构版本。网页恢复所选场次，保存修改后可自由切换。
...
手动规划编辑在镜头携带当前创意设计时可设 `applyDirectorPlan: true`。所给字段会明确应用，包括有意恢复最初规划值的情况；省略字段保持原样。写入仍绑定剧本与分镜修订，对白身份不可改写。不带该标记的旧请求保留规划与后续导演稿的原比较方式；恢复哈希覆盖含标记的原始请求。

首帧预览、生成提交和任务恢复在浏览器没有 Writer Cookie 时复用原生 Host 服务会话。已有 Cookie 保留其自身身份，认证失败不会换用服务凭据重试。同源的本人分镜确认仍仅使用 Cookie；拒绝浏览器传入 Bearer，不把服务令牌转换为本人 Cookie 或返回浏览器。

已落盘首帧候选使用 Writer 现有的媒体签名与有效期。适配器保留该单素材访问能力，将地址限定到配置的 Writer；图片元素无需账户令牌。拒绝无签名地址、无关路径和额外查询参数。回读原请求即可刷新链接，不生成或选用图片。参见[候选显示决策](../../../.agents/notes/implemented/feature/2026-09-13-qingmu-first-frame-playback.zh.md)。

首帧预览支持明确的工作图片绑定，包含原图SHA、顺序和用途。Writer核对项目及真实图片字节，将路线写入预检，提交不能靠浏览器标记切换路线。原生候选路线复用现有图片队列，不要求或授予正式素材采用、分镜签收。旧正式参考请求和本人确认维持原有边界。

场景规划读取提供影片画幅与每镜当前场景 ID。空值表示未绑定，字段缺失兼容旧版读取。客户端用这些身份定位共用场景取景，规划读取不修改布局或媒体。
