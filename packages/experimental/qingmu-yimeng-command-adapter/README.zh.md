...
素材卡片将 `visualIdentity`（主体完整外观、结构与布局）和 `imagePrompt`（本次画面或修改要求）分开保存。修图保留后续分镜沿用的主体设定。旧数据读取保留原描述；旧客户端保存时未传新字段，会保留已有主体设定。明确修改主体设定会保留已选媒体，并使未提交报价失效。两份描述不同时会一同进入图片请求。
...
原生素材卡片区分剧本事实、导演推导和世界设定例外。每张图保留相关设计依据、视角，以及最多九张按顺序绑定版本哈希和用途的项目参考；可选框选使用原图像素。万相编辑通过现有任务队列接收这些图片及描述，来源变化使报价失效。修改描述保留已选图片并提示比较，生成候选不替换原图。导演写定的场景视角和所需文字保持原意，过长请求明确失败而不截断。参考预览和报价不上传或生成媒体。 每张卡片可以单独选择素材画幅；自动模式下新图沿用项目画幅，修改沿用最后一张参考图画幅。报价、模型请求和刷新恢复保留该选择。完整设计背景与本图可见状态分开组织，后续剧情状态不作为同时绘制的要求。
