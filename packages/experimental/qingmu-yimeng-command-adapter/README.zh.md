...
`queueProductionTake` 只接受准确项目、剧集、故事板、镜头、Take 类型/编号和明确 Ready 确认。Host 自行重读当前 Ready PromptIR 与首帧报价，运行当前 PromptIR 方法，验证 HMAC 证明以及 D/E 字段、合同、卡片、来源、提示绑定，再组装 Writer 的准确九字段请求。浏览器不能提供 owner、Ready、选择、批准、force、Provider、模型或路由权威。Host 按镜头与 Take 编号派生确定性幂等键；丢响应、刷新或 Host 重启后都重放同一语义命令，并且只接受匹配的 Writer 回执。界面只开放 Take 1 `initial` 与 Take 2 `targeted_rework`；伪造的 Take 3 意图仍会到达 Writer 的持久上限，并由 Writer 拒绝。此桥不调用 Provider、不启动 Worker，也不把排队写成已经生成。参见[决策记录](../../../.agents/notes/implemented/feature/2026-09-02-qingmu-production-take-bridge.zh.md)。
