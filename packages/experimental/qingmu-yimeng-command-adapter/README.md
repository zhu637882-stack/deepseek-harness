...
The [local-reference candidate operations](src/local-reference-candidate.ts) list, upload, recover, or read one existing actor, scene, or prop image candidate. The Host accepts one strict PNG/JPEG/WebP payload of at most 3 MiB, computes its byte SHA-256, and uses a GET-only receipt lookup after an unknown result. Yimeng owns the authenticated entity scope, decoded-image checks, atomic local materialization, canonical `assets` row, ChangeSet/outbox, and receipt. The upload receipt permanently records that rights were not yet recorded; after the sole authoritative rights ChangeSet saves a knowledge record, the list projects `recorded_unverified` while the asset remains `Unselected` and unapproved. Candidate operations never call reference selection, PromptIR, a Provider, or a production stage.

The [scene-planning operations](src/scene-planning.ts) read, save or recover one imported scene's planning structure. The Host binds script revision/full SHA, storyboard revision/SHA and exact request digest. Yimeng atomically persists text entities, frames, a real structural snapshot and its existing ChangeSet/outbox/receipt records. Recovery returns original IDs without another POST. Only an absent storyboard can be initialized; an existing planning shot uses the canonical Edit mutation. No prompt, reference media, approval or production stage is created. The [decision record](../../../.agents/notes/implemented/feature/2026-08-29-qingmu-scene-entry.md) explains the structural Ready distinction.
...
The Director provider execution seam is Host-only and disabled by default. An isolated deployment may select either the credential-free loopback mock transport or the separately validated production-capable transport, never both. Production-capable configuration fixes `deepseek-official`, `deepseek-v4-pro`, `https://api.deepseek.com/chat/completions`, no reasoning, no tools or files, `maxRetries: 0`, and bounded input/output tokens; it remains inert until private configuration explicitly enables one already-issued task and method binding. Disabled configuration can issue the immutable Yimeng work order and read its private binding for a pre-submit lock without claiming a dispatch, creating a reservation or submission outbox, mounting or reading the external credential file, or invoking transport. The conservative input bound uses UTF-8 prompt bytes as a token-count upper bound, is stored in the work order, and is checked again before `prepareCall`. When transport is enabled, the Host preserves the existing binding → prepare → transport → complete/unknown sequence and consumes one `ctx.llm.prepareCall()` handle with exactly one `prepared.stream()` call. A successful result must carry stable actual completion id and finish reason, the response request id, token/cache usage, and input/output lineage; missing, drifting, or malformed facts become submission-unknown and are never retried. This seam does not expose the execution key, claim, permit, Provider payload, or credential to browser RPC, and it creates no second task or cost ledger.
