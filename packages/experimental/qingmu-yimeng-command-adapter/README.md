...
The Director provider execution seam is Host-only and disabled by default. An isolated deployment may set `directorDshTransportEnabled` together with one already-issued task, its method binding, and an exact credential-free HTTP loopback mock origin; both launcher and prepared transport reject remote endpoints, endpoint drift, and redirects. The Host then preserves the existing binding → prepare → transport → complete/unknown sequence and executes the signed `deepseek-v4-pro` text payload through one `ctx.llm.prepareCall()` handle and exactly one `prepared.stream()` call. That call requires the adapter's normal retry policy with `maxRetries: 0`, sends JSON Output without tools, images, files, attachments, agents, sessions, or a tool loop, and accepts only `qingmu.director-proposal.v1`. A successful result must carry stable actual completion id and finish reason, the response request id, token/cache usage, and input/output lineage; missing, drifting, or malformed facts become submission-unknown and are never retried. This seam does not register a production route, expose the execution key, claim, permit, or Provider payload to browser RPC, or create a second task or cost ledger.
