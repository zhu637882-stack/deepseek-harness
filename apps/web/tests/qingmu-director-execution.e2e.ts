// Real launcher/FastAPI/Host acceptance for the inactive, fake-only Director execution seam.
...
describe.skipIf(!writer || !core || process.env.DSH_CLIENT_BUILD_PROFILE !== 'qingmu')(
  'real Director Host execution identity', () => {
    it('lets a browser issue/read while only the signed Host can execute and recover one settled fake result', async () => {
      ...
      writeFileSync(join(parent, 'ACCEPTANCE-ONLY'),
        'Isolated fake Director execution. No Provider network, content approval, or user-instance write.\n')
...
print(json.dumps({"scope":scope,"counts":{t:one("SELECT count(*) FROM "+t) for t in tables},"tasks":c.execute("SELECT id,local_status,provider_status,dispatch_epoch FROM generation_tasks").fetchall(),"outbox":c.execute("SELECT dispatch_digest,state,terminal_outcome,response_json FROM provider_submission_outbox").fetchall(),"reservations":c.execute("SELECT dispatch_digest,released,amount_cny FROM provider_authorization_reservations").fetchall()}))`,
      ...
      try {
        ...
        const fixture: Record<string, unknown> = {
          fixtureOnly: true, provider: 'fake', model: 'qwen3.7-plus-2026-05-26',
        ...
        expect(order).toMatchObject({ provider: 'fake', model: fixture.model })
        ...
        fixture.transportResult = {
          providerCompletionId: 'fake-completion-browser-1', providerRequestId: 'fake-request-browser-1',
          finishReason: 'stop', usage: { promptTokens: 20, cacheTokens: 0, completionTokens: 10, totalTokens: 30 },
          proposal: { schema: 'qingmu.director-proposal.v1', ...scope, advisoryOnly: true,
            items: [{ id: 'narrative-browser-1', field: 'narrative',
              proposedValue: '让开门动作承担情绪转折', impact: '仅为隔离 fake 建议，不授予正式状态' }] },
        }
        patchConfig(fixture)
        expect(run('start').ready).toBe(true); run('login')
        ...
        expect(status!).toMatchObject({ generationTaskId: order.generationTaskId,
          executionReceipt: { providerCompletionId: 'fake-completion-browser-1', providerRequestId: 'fake-request-browser-1' },
